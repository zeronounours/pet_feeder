...
rotate([ROTATE_WHOLE * 90, 0, 0]) {
    ...
    if (SHOW_DISPENSER) color("red") translate([0, tie_length + tube_radius, tube_length - opening_size / 2 - thickness]) rotate([90, 0, 0]) dispenser();
    ...
    if (SHOW_MAIN_CASE) color("white") translate([0, spinner_height, -main_case_lower_depth]) rotate([90, 0, 0]) {
        if (SHOW_MAIN_CASE_LOWER) lower_main_case();
        if (SHOW_MAIN_CASE_UPPER) upper_main_case();
    }
